...
def generate_npm_feeder_import_file_line9(mapping_path, template_path, output_path):
    mapping_file = Path(_clean_path(mapping_path))
    if not mapping_file.is_file():
        raise ServiceError(f"File Excel feeder mapping tidak ditemukan:\n{mapping_file}", title="File tidak ditemukan")
    if mapping_file.suffix.lower() not in {".xlsx", ".xlsm"}:
    ...
    return _generate_npm_import_from_records_line9(
        mapping_records=mapping_records,
        duplicate_rows=duplicate_rows,
        mapping_file_name=mapping_file.name,
        template_path=template_path,
...
def generate_npm_feeder_import_batch_from_groups_line9(mapping_path, template_path, output_dir_path):
    mapping_file = Path(_clean_path(mapping_path))
    if not mapping_file.is_file():
        raise ServiceError(f"File Excel feeder mapping tidak ditemukan:\n{mapping_file}", title="File tidak ditemukan")
    if mapping_file.suffix.lower() not in {".xlsx", ".xlsm"}:
    ...
    for worksheet in workbook.worksheets:
        ...
        # Generate main NPM TXT (FIXED components)
        out_txt_path = output_dir / f"{clean_filename}.txt"
        res = _generate_npm_import_from_records_line9(
            mapping_records=fixed_records,
            duplicate_rows=duplicates,
            mapping_file_name=mapping_file.name,
            template_path=template_path,
        ...
        if sub_records:
            from services import feeder_mapping_service_khusus_sub
            sub_txt_path = output_dir / f"{clean_filename}_SUB.txt"
            sub_res = feeder_mapping_service_khusus_sub._generate_npm_import_from_records_line9(
                mapping_records=sub_records,
                duplicate_rows=[],
                mapping_file_name=mapping_file.name,
                template_path=template_path,
...
KNOWN_LINE9_PART_FEEDERS = {
    ...
    "0": "0.000",
    "READ": "OffsetX",
    "InitialGenerateAuthor": "LastEditAuthor",
    "PhysicalClass": "ChipL",
    "1": "0.000",
    "2": "0.000",
    "4": "5.900",
    ...
    "SIZE": "SPX",
    "NR": "NOZZLEC",
    "MntNzlLowDownDistance": "MntLoad",
    "PuNOZZLE": "PuNzlLowDownSpeed",
    "DaNOZZLE": "DaHandDaNzlVacOnTiming",
    "NAME": "T",
    "ROT": "BarCodeTypeX",
}

def _infer_npm_feeder_id_line9(part_number: str, uses_lr_position: bool = True, spans_slots: int = 1) -> str:
    key = _part_key(part_number)
    if key in KNOWN_LINE9_PART_FEEDERS:
        return KNOWN_LINE9_PART_FEEDERS[key]
    if key in KNOWN_LINE8_PART_FEEDERS:
        return KNOWN_LINE8_PART_FEEDERS[key]
    return _infer_npm_feeder_id(part_number, uses_lr_position, spans_slots)

def _feeder_id_for_import_location_line9(part_key: str, part_row: dict, feeder_lookup: dict, uses_lr_position: bool, existing_assignments: dict, spans_slots: int = 1):
    feeder_id = part_row.get("FA", "")
    if feeder_id and feeder_id not in ("0", "-1"):
        feeder_id = str(feeder_id).strip()
    else:
        feeder_id = _infer_npm_feeder_id_line9(part_key, uses_lr_position, spans_slots)
    
    if feeder_id and feeder_id in feeder_lookup:
        return feeder_id
...
def _generate_npm_import_from_records_line9(mapping_records, duplicate_rows, mapping_file_name, template_path, output_path, allow_slot_conflict=False):
    output = Path(output_path)
    if output.suffix.lower() != ".txt":
        output = output.with_suffix(".txt")
    output.parent.mkdir(parents=True, exist_ok=True)
    ...
    for record in sorted(mapping_records, key=_import_mapping_priority):
        ...
        feeder_id = _feeder_id_for_import_location_line9(
            part_key,
            part_row,
            feeder_lookup,
            record["uses_lr_position"],
...
def _feeder_id_for_import_location_line9(part_key, part_row, feeder_lookup, uses_lr_position, template_assignments, spans_slots=1):
    if part_key in template_assignments:
        ass_list = template_assignments[part_key]
        if ass_list and "feeder_id" in ass_list[0]:
            return ass_list[0]["feeder_id"]
