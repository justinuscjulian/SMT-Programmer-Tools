...
class BomComparePage(WorkerPage):
    ...
    def _build_ui(self):
        ...
        self.register_busy_widgets(self.compare_btn, self.clear_btn, self.ref_browse_btn, self.raw_browse_btn)

    def _build_reference_card(self):
        card = Card()
        header = QHBoxLayout()
        title = QLabel("BOM Excel (.txt)")
    ...
    def _build_raw_card(self):
        card = Card()
        header = QHBoxLayout()
        title = QLabel("BOM File (.tsv/.xlsx/.xls)")
        title.setObjectName("SectionTitle")
        ...
        self.raw_model = RecordTableModel(self.raw_columns, theme=self.theme_manager.theme)
    ...
    def _build_results_card(self):
        ...
        self.result_model = RecordTableModel(
            [
                ...
                ColumnSpec("tsv_part", "Part No (Source)", Qt.AlignCenter, 190),
    ...
    def _on_raw_loaded(self, file_path, result):
        ...
        self.raw_model.set_columns(columns)
    ...
    def compare_data(self):
        if self.reference_df is None or self.raw_df is None:
            ...
            return
        self.run_worker(lambda: bom_service.compare_bom(self.reference_df, self.raw_df), self._on_compare_done, "Running comparison...")
    ...
    def export_results(self):
        ...
        if not file_path.lower().endswith(".xlsx"):
            file_path += ".xlsx"
        self.run_worker(lambda path=file_path: bom_service.export_bom_results(self.diff_results, path), lambda _: QMessageBox.information(self, "Success", f"Exported to:\n{file_path}"), "Exporting results...")
    ...
    def clear_all(self):
        ...
        self.raw_model.set_columns(self.raw_columns)
